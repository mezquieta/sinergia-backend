...
export default async function handler(request, response) {
    // Habilitar CORS para que tu GitHub Pages pueda consultar sin bloqueos
    ...
    try {
        // La API Key se lee de forma 100% segura desde el servidor de Vercel
        const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
        const { action, textInput, level, systemInfo, currentPrompt } = request.body;

        // ACCIÓN A: Generar Escenario Sorpresa Oculto
        ...
        // ACCIÓN B: Evaluar la respuesta de Voz Real
